...
class troop:
    ...
    def troop_damage(self, damage, village_matrix, vill_index):
        self.curr_hp -= damage
        if (self.curr_hp <= 0):
...
class king(troop):
    ...
    def damage(self, damage, village_matrix, vill_index):
        village_matrix, vill_index = self.troop_damage(damage, village_matrix, vill_index)
        return village_matrix, vill_index
            ...
            for i in range(len(my_village.building_array)):
                for j in range(len(my_village.building_array[i])):
                    ...
                    if (my_village.building_array[i][j].code == code):
                        required_building = my_village.building_array[i][j]
                        break
            required_building.damage(self.damage, village_matrix, vill_index)
        # return village_matrix, vill_index        
